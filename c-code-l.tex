...
struct pixel
{
  ...
  int p_co[|\trois|];             // the pixel coordinates in the grid
...
\begin{lstlisting}[language=C]
void set_pbc_shift (grid pixel_grid, int pixel_coord[|\trois|], int pbc_shift[|\trois|][|\trois|][|\trois|])
{
  ...
  else if ( pixel_coord[|\zero|] == pixel_grid->n_pix[|\zero|] - |\un| ) // pixel position on 'x' is max
  {
    for ( |\rbtt{y\_pos}| = |\zero| ; |\rbtt{y\_pos}| < |\trois| ; |\rbtt{y\_pos}| ++ )
    {
      for ( |\obtt{z\_pos}| = |\zero| ; |\obtt{z\_pos}| < |\trois| ; |\obtt{z\_pos}| ++ )
  ...
  else if ( pixel_coord[|\un|] == pixel_grid->n_pix[|\un|] - |\un| ) // pixel position on 'y' is max
  {
    for ( |\vbtt{x\_pos}| = |\zero| ; |\vbtt{x\_pos}| < |\trois| ; |\vbtt{x\_pos}| ++ )
    {
      for ( |\obtt{z\_pos}| = |\zero| ; |\obtt{z\_pos}| < |\trois| ; |\obtt{z\_pos}| ++ )
  ...
  else if ( pixel_coord[|\deux|] == pixel_grid->n_pix[|\deux|] - |\un| ) // pixel position on 'z' is max
  {
    for ( |\vbtt{x\_pos}| = |\zero| ; |\vbtt{x\_pos}| < |\trois| ; |\vbtt{x\_pos}| ++ )
    {
      for ( |\rbtt{y\_pos}| = |\zero| ; |\rbtt{y\_pos}| < |\trois| ; |\rbtt{y\_pos}| ++ )
...
// - grid * the_gird : pointer to the pixel grid
// - pixel * the_pix : pointer to the pixel with neighbors to be found
void find_pixel_neighbors (bool use_pbc, grid * the_grid, pixel * the_pix)
{
...
if ( use_pbc )
  ...
  else
  {
    ...
    {
      if ( the_pix->p_co[|\dbtt{axis}|] == |\zero| |\textbar\textbar| the_pix->p_co[|\dbtt{axis}|] == the_grid->n_pix[|\dbtt{axis}|] - |\un| ) boundary = TRUE;
    }
  }
  for ( |\dbtt{axis}| = |\zero| ; |\dbtt{axis}| < |\trois| ; |\dbtt{axis}| ++ )
  {
  ...
  for ( |\vbtt{xpos}| = l_start[|\zero|] ; |\vbtt{xpos}| < l_end[|\zero|] ; |\vbtt{xpos}| ++ )
  {
    for ( |\rbtt{ypos}| = l_start[|\un|] ; |\rbtt{ypos}| < l_end[|\un|] ; |\rbtt{ypos}| ++ )
    {
      for ( |\obtt{zpos}| = l_start[|\deux|] ; |\obtt{zpos}| < l_end[|\deux|] ; |\obtt{zpos}| ++ )
      {
        ...
        if ( ! use_pbc && boundary )
        {
          if (( the_pix->p_co[|\zero|] == |\zero| && |\vbtt{xpos}| == |\zero| ) |\textbar\textbar| ( the_pix->p_co[|\zero|] == the_grid->n_pix[|\zero|] && |\vbtt{xpos}| == |\deux| ))
          {
            keep_neighbor = FALSE;
          }
          else if (( the_pix->p_co[|\un|] == |\zero| && |\rbtt{ypos}| == |\zero| ) |\textbar\textbar| ( the_pix->p_co[|\un|] == the_grid->n_pix[|\un|] && |\rbtt{ypos}| == |\deux| ))
          {
            keep_neighbor = FALSE;
          }
          else if (( the_pix->p_co[|\deux|] == |\zero| && |\obtt{zpos}| == |\zero| ) |\textbar\textbar| ( the_pix->p_co[|\deux|] == the_grid->n_pix[|\deux|] && |\obtt{zpos}| == |\deux| ))
          {
            keep_neighbor = FALSE;
          }
        }
...
pixel_grid * prepare_pixel_grid (bool use_pbc)
{
  ...
  if ( ! use_pbc ) // Without periodic boundary conditions
  {
    for ( |\cbtt{aid}| = |\zero| ; |\cbtt{aid}| < atoms ; |\cbtt{aid}| ++ )  // For all atoms
    {
      ...
      // - Add atom '|\cbtt{aid}|' with coordinates 'c_coord[|\cbtt{aid}|]' to pixel 'pixel_number'
