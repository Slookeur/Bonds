...
\newcommand{\boxA}{\text{A}}
\newcommand{\boxB}{\text{B}}
\newcommand{\boxC}{\text{C}}
\newcommand{\Tc}{{\text{\bf{T}}_{\bf{c}}}}
\newcommand{\Tf}{{\text{\bf{T}}_{\bf{f}}}}
\newcommand{\Rc}{\text{R}_{cut}}
\newcommand{\Pc}{\text{R}_{pix}}
\newcommand{\Dij}{{\text{D}_{ij}}}
...
\title{Efficient first neighbor analysis in large atomic scale models [Article v\versionnumber]}

% Do I really need the number ?
\author[1*\authfn{1}]{Sébastien Le Roux}
\affil{Institut de Physique et Chimie des Matériaux de Strasbourg, Université de Strasbourg, CNRS UMR 7504, 23 rue de Loess, BP 43, F-67034 Strasbourg Cedex 2, France}

\corr{sebastien.leroux@ipcms.unistra.fr}{SLR}
...
\begin{abstract}
This tutorial article describes how lattice mathematical properties can be combined with the 3D space pixelation method 
to evaluate interatomic distances in large atomic scale 3D models with the purpose of searching for first neighbor atoms. 
It aims to be an educational tool for students and researchers who want to develop structural analysis or 3D visualization tools 
that requires to implement and compute efficiently the search for first neighbor atoms. \\
Examples codes are provided in C, FORTRAN90 and Python. 
\end{abstract}
...
when dealing with the periodicity of non-cubic systems, and even more dramatically with the search for performance that is driving the analysis
...
of the calculation of interatomic bond distances in 3D atomic scale models using {\bf{P}}eriodic {\bf{B}}oundary {\bf{C}}onditions. \\
...
\noindent Periodicity can expressed with two different sets of parameters, using either: 
\begin{itemize}
\item[1.]\label{abc} the box parameters \boxA, \boxB, \boxC\ and the associated angles $\alpha$, $\beta$ and $\gamma$
\item[2.]\label{uvw} the components of the lattice vectors \\
$\ \vec{a} (a_x, a_y, a_z)$, $\ \vec{b} (b_x, b_y, b_z)\ $ and $\ \vec{c} (c_x, c_y, c_z)$
\end{itemize}
Lattice vectors can be calculated from the box parameters by: \\ 
\begin{equation}
...
\end{equation}
\\
While box parameters can be calculated from the lattice vectors by: 
\begin{equation}
...
that allow to convert Cartesian $r$ to Fractional $f$ coordinates, and vice versa, Fractional $f$ to Cartesian $r$ coordinates. 
...
\noindent Frequently used in MD simulations under the name linked cell algorithm(s) \cite{CSL,WELLING2011611}, 
the idea of pixelation, or partitioning, of the model box illustrated in this section is mandatory to deal efficiently with 
searching for first neighbor atoms in large atomic scale models.\\[0.125cm] 
...
distance $\Dij$ between $i$ ($\alpha$) and $j$ ($\beta$), and then compared this distance to a cutoff radius $\Rc(\alpha,\beta)$, 
that could appropriately be determined when looking at the radial distribution function $g_{\alpha\beta}(r)$. 
Then if $\Dij$ is smaller or equal to $\Rc(\alpha,\beta)$ then atoms $i$ and $j$ are first neighbors, otherwise they are not. 
For a program which purpose is to render the atomic scale model in 3D space, the result of the analysis would then be to draw, or not, a bond between atoms $i$ and $j$. \\
...
Which, as long as the size of the system remains within the thousand or few thousands of atoms range, could work in a seemingly efficient manner. 
...
Therefore another approach is required to optimize the procedure for large atomic scale models, and that is 
...
Then for this particular atom, first neighbors candidates will only be search for in that same pixel and its immediate surrounding pixel neighbors. 
Ideally the pixel dimension $\Pc$ should be close but slightly superior to $\Rc$ the cutoff radius used to search for first neighbors atoms, ex: $\Pc = \Rc + 0.5$. \\
\\ 
For any atom in the model this approach will limit the area of interest to the smallest possible size. 
...
\begin{equation}
\label{e_npix}
n_{p}(axis) = \left\lceil \frac{D_{max}(axis)}{\Pc} \right\rceil 
\end{equation}
Where $D_{max}(axis)$ is the maximum interatomic distance separating two atoms on $axis$, and $\Pc \simeq \Rc$ is the pixel dimension, close to the cutoff distance that separates first neighbor atoms. \\
...
It is then required to test each and every pair of atomic coordinates in the model on $x$, $y$ and $z$. 
However as long as only subtractions and min/max comparisons are involved calculation time will remain completely acceptable. \\[0.25cm]
...
\begin{equation}
p_{axis} = \left\lfloor \frac{r_{axis} - min_{axis}}{\Pc} \right\rfloor
\end{equation}
With:
\begin{equation}
...
Using fractional coordinates is the easiest way to propose a general methodology to compute interatomic distances in the model.  
Indeed this requires to consider the periodicity of the system, and, in the case of non-cubic symmetry transformations, could be tricky when using Cartesian coordinates. 
...
\begin{equation}
f_{c,axis} = f_{axis} - \left\lfloor\ f_{axis}\ \right\rfloor
\end{equation}
With:
\begin{equation}
0 \le f_{c,axis} < 1
...
\begin{equation}
p_{axis} = \left\lfloor\ f_{c,axis} \times n_{p}(axis)\ \right\rfloor
\end{equation}
With:
\begin{equation}
p_{axis} \in [0, n_{p}(axis)-1]
\end{equation}
\item[4.] Determine each pixel neighbors: 
\begin{itemize}
\item Pixel inside the pixel grid: \\
\begin{center}
\includegraphics[width=\pwdi]{img/pixel-27-2.eps} \\[0.25cm]
Self + 26 neighbors, no PBC transformation required.\\[0.5cm] 
\end{center}
\newpage
\item Pixel on the boundary of the pixel grid :\\
$p_{axis} = 0$ or $p_{axis} =  n_{p}(axis)-1$\\ 
...
\end{center}
\end{itemize}
\noindent For a pixel with number $p_{id}$ with pixel coordinates ($p_x$, $p_y$, $p_z$) inside the pixel grid, pixel neighbors are determined as illustrated in figure~\ref{pixelmap}. 
For a pixel on the boundary of the grid, as the 3 cases described above, that means if $p_{axis} = 0$ or $p_{axis} = n_{p}(axis) - 1$, 
then it is possible to determine its PBC pixel neighbors by applying a shift to the pixel neighbors number calculated as illustrated in fig.~\ref{pixelmap}. \\
In the in the 3 directions of space: 
\begin{align}\nonumber
{\text{if }} p_{x} & = & 0 & \quad {\text{shift all}} & p_{x}-1 & \quad {\text{pixels by}} & n_{p}(x) \\ \nonumber
{\text{if }} p_{x} & = & n_{p}(x) - 1 & \quad {\text{shift all}} & p_{x}+1 & \quad {\text{pixels by}} & -n_{p}(x) \\ \nonumber
{\text{if }} p_{y} & = & 0 & \quad {\text{shift all}} & p_{y}-1 & \quad {\text{pixels by}} & n_{p}(xy) \\ \nonumber
{\text{if }} p_{y} & = & n_{p}(y) - 1 & \quad {\text{shift all}} & p_{y}+1 & \quad {\text{pixels by}} & -n_{p}(xy) \\ \nonumber
{\text{if }} p_{z} & = & 0 & \quad {\text{shift all}} & p_{z}-1 & \quad {\text{pixels by}} & n_{p}(xyz) \\ \nonumber
{\text{if }} p_{z} & = & n_{p}(z) - 1 & \quad {\text{shift all}} & p_{z}+1 & \quad {\text{pixels by}} & -n_{p}(xyz) \nonumber
\end{align}
With: 
\begin{equation}
n_{p}(xy) = n_{p}(x) \times n_{p}(y)
\end{equation}
And: 
\begin{equation}
n_{p}(xyz) = n_{p}(xy) \times n_{p}(z)
\end{equation}
\vspace{0.125cm}
\item[5.] When searching for first neighbor atoms between atoms $i$ and $j$ of the same pixel or one of its surrounding pixel neighbor, 
compute the interatomic distance $\Dij$ between $i$ and $j$ using their corrected fractional coordinates:
\begin{eqnarray}
f_{ij}(axis) & = & f_{c,axis}(i) - f_{c,axis}(j) \\[0.125cm]
f_{c,ij}(axis) & = & f_{ij}(axis) - \left\lfloor\ f_{ij}(axis)\ \right\rceil \\[0.125cm]
\vec{r_{ij}} & = & \Tc \times \vec{f_{c,ij}} \\[0.125cm]
\quad {\text{with}} \quad \vec{r_{ij}} & = & \left( \begin{blockarray}{c}
r_{ij}(x) \\
r_{ij}(y) \\
r_{ij}(z) 
\end{blockarray} \right) \\[0.125cm]
\quad {\text{and}} \quad \vec{f_{c,ij}} & = & \left( \begin{blockarray}{c}
f_{c,ij}(x) \\
f_{c,ij}(y) \\
f_{c,ij}(z) 
\end{blockarray} \right) \\[0.125cm]
\Dij & = & \lvert \vec{r_{ij}} \rvert
\end{eqnarray}
Note that some further calculation time can be spared if $\Dij^2$ only is calculated so that no squared root calculation is performed to obtain $\Dij$. 
Therefore in that case $\Dij^2$ must be compared to $\Rc^2$. \\
...
In the case where PBC are applied this means that the number of pixels on one dimension, $x$, $y$, or $z$ should be higher than 3. \\[0.25cm]
\end{itemize}

\section{Limitations}

Overall the pixelation, or partitioning, of the model box approach is extremely efficient. 
However some issues might arise when dealing with a very large system containing very few atoms, which could happen if: 
\begin{itemize}
\item No periodicity is involved but the model is very large and describes a single, or few, isolated molecule(s) that are far apart.
\item The description of the model relies on a very large box, usually with a simple cubic periodicity, that contains a single or few isolated molecule. 
\end{itemize}
In such rare cases the number density, in atoms / \AA$^3$, can be very low  (< 0.01  atom / \AA$^3$), 
but the number of pixels required to build the entire pixel grid can be too high for the memory allocation(s) required to perform the analysis. 
To overcome this issue the size of the pixel $\Pc$ can be increased, to a value significantly higher than the cutoff radius $\Rc$, 
so that the density of atom(s) per pixel can be adjusted to match a proper target value to ensure that memory allocation will not be an issue,
and that the analysis will be successful. By experience a value between 1.5 and 2 atoms per pixel is a safe window to avoid any issue in these peculiar cases. 
...
The methodology described in this article is the one implemented in the \atomes\ software \cite{CMSatomes}. 
...
\begin{itemize}
\item Choose a cutoff radius $\Rc$ to determine first neighbor atoms, and the associated pixel dimension $\Pc$
\item Find the maximum distances, $D_{max}(x)$, $D_{max}(y)$ and $D_{max}(z)$ that separate 2 atoms on $x$, $y$ and $z$
\item Determine the number of pixels, $n_{p}(axis)$, on $x$, $y$ and $z$ using $n_{p}(axis) = \left\lceil \frac{D_{max}(axis)}{\Pc} \right\rceil$
\item Create a pixel grid for a virtual box with dimensions $n_{p}(x)$, $n_{p}(y)$ and $n_{p}(z)$ on $x$, $y$ and $z$ axis respectively.
\item Using its Cartesian coordinates, $r(at)$, associate each atom $at$ with a pixel position in the grid
\begin{itemize}
\item Pixel position on each $axis$: $p_{axis} = \left\lfloor \frac{r_{at}(axis) - min_{axis}}{\Pc} \right\rfloor$
\item Pixel ID number in the grid: $P_{id}(at) = p_{x} + n_{p}(x) \times p_{y} + \left[n_{p}(x) \times n_{p}(y)\right] \times p_{z}$
\end{itemize}
\item Associate each pixel with it surrounding neighbors in the grid
\item For all pixels in the grid
\begin{itemize}
\item Search for neighbors between the atoms of pixel $pix$ as well as the atoms of its surrounding pixel neighbors $pjx$
\begin{itemize}
\item For 2 atoms $i$ in pixel $pix$ and $j$ in pixel $pjx$
\begin{itemize}
\item Evaluate $r_{ij}$ on $x$, $y$ and $z$ using atoms $i$ and $j$ Cartesian coordinates
\item Compute and return $\Dij^2$ 
\item Compare $\Dij^2$ to $\Rc^2$ to determine if $i$ and $j$ are first neighbors
\end{itemize}
\end{itemize}
\end{itemize}
\end{itemize}
...
\item Determine the number of pixels, $n_{p}(axis)$, on $x$, $y$ and $z$ using $n_{p}(axis) = \left\lceil \frac{D_{max}(axis)}{\Rc} \right\rceil$
\item Create a pixel grid with dimensions $n_{p}(x)$, $n_{p}(y)$ and $n_{p}(z)$ on $x$, $y$ and $z$ axis respectively.
\item Convert all atom Cartesian coordinates to fractional coordinates $f$ using $\Tf$
\item Compute the corrected fractional coordinates $f_c$ to ensure that all atoms are in the unit cell $f_{c} = f - \left\lfloor\ f \right\rfloor$
\item Using its correct fractional coordinates, $f_{c}(at)$, associate each atom $at$ with a pixel position in the grid
\begin{itemize}
\item Pixel position on each $axis$: $p_{axis} = \left\lfloor f_{c,axis}(at) \times n_{p}(axis) \right\rfloor$
\item Pixel ID number in the grid: $P_{id}(at) = p_{x} + n_{p}(x) \times p_{y} + \left[n_{p}(x) \times n_{p}(y)\right] \times p_{z}$
\end{itemize}
\item Associate each pixel with it surrounding neighbors in the grid, if needed apply PBC transformation(s)
\item For all pixels in the grid
\begin{itemize}
\item Search for neighbors between the atoms of pixel $pix$ as well as the atoms of its surrounding pixel neighbors $pjx$
\begin{itemize}
\item For 2 atoms $i$ in pixel $pix$ and $j$ in pixel $pjx$
\begin{itemize}
\item Evaluate $f_{ij}$ on $x$, $y$ and $z$ using the atom $i$ and $j$ corrected fractional coordinates $f_c$
\item Correct $f_{ij}$ to $f_{c,ij}$ for potential PBC correction(s) : $f_{c,ij} = f_{ij} - \left\lfloor\ f_{ij}\ \right\rceil$
\item Transform back $f_{c,ij}$ to Cartesian coordinates $r_{ij}$ using $\Tc$
\item Compute and return $\Dij^2$
\item Compare $\Dij^2$ to $\Rc^2$ to determine if $i$ and $j$ are first neighbors
\end{itemize}
\end{itemize}
