...
The pixel dimension $a = \Rc$, with $\Rc$ is the cutoff radius used to search for first neighbors atoms. \\ 
...
\begin{equation}
\label{e_npix}
n_{p}(axis) = \left\lceil \frac{D_{max}(axis)}{\Rc} \right\rceil 
\end{equation}
Where $D_{max}(axis)$ is the maximum interatomic distance separating two atoms on $axis$, and $\Rc$ is the cutoff distance that separates neighbor atoms. \\
...
\begin{equation}
p_{axis} = \left\lfloor \frac{r_{axis} - min_{axis}}{\Rc} \right\rfloor
\end{equation}
With:
\begin{equation}
...
\begin{itemize}
\item Choose a cutoff radius $\Rc$ to determine first neighbor atoms, compute and save $\Rc^2$
...
\begin{itemize}
\item Pixel position on each $axis$: $p_{axis} = \left\lfloor \frac{r_{at}(axis) - min_{axis}}{\Rc} \right\rfloor$
