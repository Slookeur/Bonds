...
Idealy the pixel dimension $\Pc$ should be close but sligtly superior to $\Rc$ the cutoff radius used to search for first neighbors atoms, ex: $\Pc = \Rc + 0.5$. \\
\\ 
...
Using fractional coordinates is the easiest way to propose a general methodoly to compute interatomic distances in the model.  
...
Overall the pixelation, or partioning, of the model box approach is extremely efficient. 
...
and that the analysis will be successfull. By experience a value between 1.5 and 2 atoms per pixel is a safe window to avoid any issue in these peculiar cases. 
