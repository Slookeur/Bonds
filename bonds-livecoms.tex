...
but the number of pixels to describe the entire system to build the entire pixel grid can be too high for the memory allocation(s) required to perform the analysis. 
To overcome this issue the size of the pixel $\Pc$ can be increased, to a value significantly higher than the cutoff radius $\Rc$, so that the density of atom(s) per pixel can be adjusted 
to match a proper target value to ensure that the analysis will be successfull, for instance between 1.5 and 2 atoms per pixel. 
