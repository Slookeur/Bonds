...
distance D$_{ij}$ between $i$ ($\alpha$) and $j$ ($\beta$), and then compared this distance to a cutoff radius $\Rc(\alpha,\beta)$, 
that could appropriately be determined when looking at the radial distribution function $g_{\alpha\beta}(r)$. 
Then if D$_{ij}$ is smaller or equal to $\Rc(\alpha,\beta)$ then atoms $i$ and $j$ are first neighbors, otherwise they are not. 
...
compute the interatomic distance $D_{ij}$ between $i$ and $j$ using their corrected fractional coordinates:
\begin{eqnarray}
f_{c,axis}(ij) & = & f_{c,axis}(i) - f_{c,axis}(j) \\[0.125cm]
f_{axis}(ij) & = & f_{c,axis(ij)} - \left\lfloor f_{c,axis(ij)} \right\rceil \\[0.125cm]
\vec{r}(ij) & = & \Tc \times \vec{f}(ij) \\[0.125cm]
\quad {\text{with}} \quad \vec{r}(ij) & = & \left( \begin{blockarray}{c}
r_x(ij) \\
r_y(ij) \\
r_z(ij)
\end{blockarray} \right) \\[0.125cm]
\quad {\text{and}} \quad \vec{f}(ij) & = & \left( \begin{blockarray}{c}
f_{x}(ij) \\
f_{y}(ij) \\
f_{z}(ij)
\end{blockarray} \right) \\[0.125cm]
D(ij) & = & \lvert \vec{r}(ij) \rvert
\end{eqnarray}
Note that some further calculation time can be spared if $D(ij)^2$ only is calculated so that no squared root calculation is performed to obtain $D(ij)$. 
Therefore in that case $D(ij)^2$ must be compared to $\Rc^2$. \\
...
\begin{itemize}
\item Pixel position on each $axis$: $p_{axis} = \left\lfloor \frac{r_{axis}(at) - min_{axis}}{\Rc} \right\rfloor$
...
\item Compute and return ${D_{ij}}^2$ 
\item Compare ${D_{ij}}^2$ to $\Rc^2$ to determine if $i$ and $j$ are first neighbors
\end{itemize}
\end{itemize}
\end{itemize}
\end{itemize}
...
\item Transform back to Cartesian coordinates using $\Tc$
\item Compute and return ${D_{ij}}^2$
\item Compare ${D_{ij}}^2$ to $\Rc^2$ to determine if $i$ and $j$ are first neighbors
\end{itemize}
\end{itemize}
