...
\title{Efficient first neighbor analysis in large atomic scale models [Artivle v\versionnumber]}
...
\begin{itemize}
\item Choose a cutoff radius ${\textrm{R}_{cut}}$ to determine first neighbor atoms, compute and save ${\textrm{R}_{cut}}^2$
...
\item For all pixels $pix$ in the grid
\begin{itemize}
\item Search for neighbors between the atoms of pixel $pix$ as well as the atoms of its surrounding pixel neighbors $pjx$
\begin{itemize}
\item For 2 atoms $a$ in $pix$ and $b$ in $pjx$
\begin{itemize}
\item Evaluate $r_{ab}$ on $x$, $y$ and $z$ using atoms $a$ and $b$ Cartesian coordinates
\item Compute and return ${D_{ab}}^2$ 
\item Compare ${D_{ab}}^2$ to ${\textrm{R}_{cut}}^2$ to determine if $a$ and $b$ are first neighbors
\end{itemize}
\end{itemize}
\end{itemize}
\end{itemize}
...
\item Compute the corrected fractional coordinates $f_c$ to ensure that all are in the unit cell $f_{c,axis} = f_{axis} - \left\lfloor f_{axis} \right\rfloor$
...
\item For 2 atoms $a$ in $pix$ and $b$ in $pjx$
\begin{itemize}
\item Evaluate $f_{ab}$ on $x$, $y$ and $z$ using the atom $a$ and $b$ corrected fractional coordinates $f_c$
