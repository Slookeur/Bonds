...
\subsection{C code: adjusting the number of pixels in the grid}
\label{c-adjust-pixel-grid}

\begin{lstlisting}[language=C]
float adjust_pixel_numbers (bool use_pbc, pixel_grid * grid, float cmin[|\trois|], float cmax[|\trois|], float pixel_size)
{
  
  int |\dbtt{axis}|;                       // loop iterator axis id (1=x, 2=y, 3= z)
  float targetdp = |\mbtt{1.85}|;          // target atom density per pixel
  ...
  else
  {
    ...
    {
      rhonum /= (cmax[|\dbtt{axis}|] - cmin[|\dbtt{axis}|]);
    }
  }
  ...
  if (rhonum < |\mbtt{0.01}|)
  {
    ...
    for ( |\dbtt{axis}| = |\zero| ; |\dbtt{axis}| < |\trois| ; |\dbtt{axis}| ++ )      // For x, y and z
    {
      rhopix /= grid->n_pix[|\dbtt{axis}|];
    }
    mpsize = pow (targetdp/rhopix, |\mbtt{1.0}|/|\mbtt{3.0}|);
    if (mpsize > |\mbtt{1.0}|)
    {
 ...
 for ( |\dbtt{axis}| = |\zero| ; |\dbtt{axis}| < |\trois| ; |\dbtt{axis}| ++ )         // For x, y and z
 {
   ...
   grid->n_pix[|\dbtt{axis}|] = (grid->n_pix[|\dbtt{axis}|] < |\trois|) ? |\un| : grid->n_pix[|\dbtt{axis}|];
 }
 return pixel_size;
}
\end{lstlisting}
...
pixel_grid * prepare_pixel_grid (bool use_pbc)
{
  ...
  if ( use_pbc )                                // using periodic boundary conditions
  {
    for ( |\dbtt{axis}| = |\zero| ; |\dbtt{axis}| < |\trois| ; |\dbtt{axis}| ++ )       // For x, y and z
    {
      grid->n_pix[|\dbtt{axis}|] = (int)(l_params[|\dbtt{axis}|] / pixel_size); // number of pixel(s) on 'axis'
    }
  }
  else                                          // without periodic boundary conditions
  {
    ...
    for ( |\dbtt{axis}| = |\zero| ; |\dbtt{axis}| < |\trois| ; |\dbtt{axis}| ++ )       // For x, y and z
    {
      grid->n_pix[|\dbtt{axis}|] = (int)((cmax[|\dbtt{axis}|] - cmin[|\dbtt{axis}|])/pixel_size); // number of pixel(s) on 'axis'
    }
  }
  ...
  for ( pixel_num = |\zero| ; pixel_num < grid->pixels ; pixel_num ++ )
  {
    grid->pixel_list[pixel_num].pid = pixel_num;
    grid->pixel_list[pixel_num].patoms = |\zero|;
  ...
  if ( use_pbc )                                // using periodic boundary conditions
  {
    for ( |\cbtt{aid}| = |\zero| ; |\cbtt{aid}| < atoms ; |\cbtt{aid}| ++ )      // for all atoms
    {
      ...
      matrix_multiplication (f_coord, cart_to_frac, c_coord[|\cbtt{aid}|]);
      for ( |\dbtt{axis}| = |\zero| ; |\dbtt{axis}| < |\trois| ; |\dbtt{axis}| ++ )     // for x, y and z
      {
  ...
  else                                          // without periodic boundary conditions
  {
    for ( |\cbtt{aid}| = |\zero| ; |\cbtt{aid}| < atoms ; |\cbtt{aid}| ++ )      // for all atoms
    {
      for ( |\dbtt{axis}| = |\zero| ; |\dbtt{axis}| < |\trois| ; |\dbtt{axis}| ++ )     // for x, y and z
      {
        pixel_pos[|\dbtt{axis}|] = (grid->n_pix[|\dbtt{axis}|] == |\un|) ? |\zero| : (int)((c_coord[|\cbtt{aid}|][|\dbtt{axis}|] - cmin[|\dbtt{axis}|])/pixel_size);
      }
      pixel_num = pixel_pos[|\zero|] + pixel_pos[|\un|] * grid->n_pix[|\zero|] + pixel_pos[|\deux|] * grid->n_xy;
      add_atom_to_pixel (& grid->pixel_list[pixel_num], pixel_pos, pixel_pos, |\cbtt{aid}|, c_coord[|\cbtt{aid}|]);
    }
  }
  return grid;
}
...
{
  int |\dbtt{axis}|;                       // loop iterator axis id (1=x , 2=y , 3= z)
  int |\vbtt{x\_pos}|, |\rbtt{y\_pos}|, |\obtt{z\_pos}|;        // neighbor position on x, y and z
  ...
  |\bftt{nnp}| = |\zero|;
  for ( |\vbtt{x\_pos}| = l_start[|\zero|] ; |\vbtt{x\_pos}| < l_end[|\zero|] ; |\vbtt{x\_pos}| ++ )
  {
    for ( |\rbtt{y\_pos}| = l_start[|\un|] ; |\rbtt{y\_pos}| < l_end[|\un|] ; |\rbtt{y\_pos}| ++ )
    {
      for ( |\obtt{z\_pos}| = l_start[|\deux|] ; |\obtt{z\_pos}| < l_end[|\deux|] ; |\obtt{z\_pos}| ++ )
      {
        keep_neighbor = TRUE;
        if ( ! use_pbc && boundary )
        {
          if (( the_pix->p_xyz[|\zero|] == |\zero| && |\vbtt{x\_pos}| == |\zero| ) |\textbar\textbar| ( the_pix->p_xyz[|\zero|] == the_grid->n_pix[|\zero|]-|\un| && |\vbtt{x\_pos}| == |\deux| ))
          {
            keep_neighbor = FALSE;
          }
          else if (( the_pix->p_xyz[|\un|] == |\zero| && |\rbtt{y\_pos}| == |\zero| ) |\textbar\textbar| ( the_pix->p_xyz[|\un|] == the_grid->n_pix[|\un|]-|\un| && |\rbtt{y\_pos}| == |\deux| ))
          {
            keep_neighbor = FALSE;
          }
          else if (( the_pix->p_xyz[|\deux|] == |\zero| && |\obtt{z\_pos}| == |\zero| ) |\textbar\textbar| ( the_pix->p_xyz[|\deux|] == the_grid->n_pix[|\deux|]-|\un| && |\obtt{z\_pos}| == |\deux| ))
          {
            keep_neighbor = FALSE;
          }
        }
        if ( keep_neighbor )
        {
          ...
          nid = the_pix->pid + pmod[|\vbtt{x\_pos}|] + pmod[|\rbtt{y\_pos}|] * the_grid->n_pix[|\zero|] + pmod[|\obtt{z\_pos}|] * the_grid->n_xy;
          // corrections if required in the case where PBC are applied
          if ( use_pbc ) nid += pbc_shift[|\vbtt{x\_pos}|][|\rbtt{y\_pos}|][|\obtt{z\_pos}|];
      ...
      for ( pid = |\zero| ; pid < pix_i->neighbors ; pid ++ )
      {
        ...
        if ( pix_j->patoms && ! pix_j->tested )
        {
          ...
          for ( |\dbtt{aid}| = |\zero| ; |\dbtt{aid}| < pix_i->patoms - l_end ; |\dbtt{aid}| ++ )
          {
            ...
            l_start = (|\rbtt{pjx}| != |\vbtt{pix}|) ? |\zero| : |\dbtt{aid}| + |\un|;
            // for all atom(s) in 'pix_j'
            for ( |\cbtt{bid}| = l_start ; |\cbtt{bid}| < pix_j->patoms ; |\cbtt{bid}| ++ )
            {
