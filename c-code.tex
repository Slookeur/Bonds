...
void find_pixel_neighbors (bool use_pbc, pixel_grid * the_grid, pixel * the_pix)
{
  ...
  |\bftt{nnp}| = |\zero|;
  for ( |\vbtt{x_pos}| = l_start[|\zero|] ; |\vbtt{x_pos}| < l_end[|\zero|] ; |\vbtt{x_pos}| ++ )
  {
    for ( |\rbtt{y_pos}| = l_start[|\un|] ; |\rbtt{y_pos}| < l_end[|\un|] ; |\rbtt{y_pos}| ++ )
    {
      for ( |\obtt{z_pos}| = l_start[|\deux|] ; |\obtt{z_pos}| < l_end[|\deux|] ; |\obtt{z_pos}| ++ )
      {
        keep_neighbor = TRUE;
        if ( ! use_pbc && boundary )
        {
          if (( the_pix->p_xyz[|\zero|] == |\zero| && |\vbtt{x_pos}| == |\zero| ) |\textbar\textbar| ( the_pix->p_xyz[|\zero|] == the_grid->n_pix[|\zero|] && |\vbtt{x_pos}| == |\deux| ))
          {
            keep_neighbor = FALSE;
          }
          else if (( the_pix->p_xyz[|\un|] == |\zero| && |\rbtt{y_pos}| == |\zero| ) |\textbar\textbar| ( the_pix->p_xyz[|\un|] == the_grid->n_pix[|\un|] && |\rbtt{y_pos}| == |\deux| ))
          {
            keep_neighbor = FALSE;
          }
          else if (( the_pix->p_xyz[|\deux|] == |\zero| && |\obtt{z_pos}| == |\zero| ) |\textbar\textbar| ( the_pix->p_xyz[|\deux|] == the_grid->n_pix[|\deux|] && |\obtt{z_pos}| == |\deux| ))
          {
            keep_neighbor = FALSE;
          }
        }
        if ( keep_neighbor )
        {
          nid = the_pix->pid + pmod[|\vbtt{x_pos}|] + pmod[|\rbtt{y_pos}|] * the_grid->n_pix[|\zero|] + pmod[|\obtt{z_pos}|] * the_grid->n_xy;
