...
pixel_grid * prepare_pixel_grid (bool use_pbc)
{
  ...
  if ( use_pbc )                    // using periodic boundary conditions
  {
    ...
    {
      grid->n_pix[|\dbtt{axis}|] = (int)(l_params[|\dbtt{axis}|] / cutoff) + |\un|; // number of pixel(s) on axis '|\dbtt{axis}|'
    }
  }
  else                              // without periodic boundary conditions
  {
    ...
    for ( |\dbtt{axis}| = |\zero| ; |\dbtt{axis}| < |\trois| ; |\dbtt{axis}| ++ )       // For x, y and z
    {
      grid->n_pix[|\dbtt{axis}|] = (int)((cmax[|\dbtt{axis}|] - cmin[|\dbtt{axis}|]) / cutoff) + |\un|; // number of pixel(s) on axis '|\dbtt{axis}|'
    }
  }
  for ( |\dbtt{axis}| = |\zero| ; |\dbtt{axis}| < |\trois| ; |\dbtt{axis}| ++ )         // For x, y and z
  {
