...
\Procedure{prepare\_pixel\_grid}{use\_pbc, l\_params[\trois], cutoff, c\_cooord[][\trois]}
  ...
  \If{use\_pbc}
    \For{\dbtt{axis} = \un, $\dots$, \trois}
      \State grid->n\_pix[\dbtt{axis}] = $\left\lfloor (l\_params[\dbtt{axis}] / cutoff) \right\rfloor$ + \un
    \EndFor
  \Else
    \For{\dbtt{axis} = \un, $\dots$, \trois}
      \State cmin = cmax = c\_coord[\un][\dbtt{axis}]
    \EndFor
    \For{\dbtt{aid} = \deux, $\dots$, atoms}
      \For{\dbtt{axis} = \un, $\dots$, \trois}
    ...
    \EndFor
    \For{\dbtt{axis} = \un, $\dots$, \trois}
      \State grid->n\_pix[\dbtt{axis}] = $\left\lfloor (cmax[\dbtt{axis} - cmin[\dbtt{axis}] / cutoff) \right\rfloor$ + \un
    \EndFor
  \EndIf
  \State
  \For{\dbtt{axis} = \un, $\dots$, \trois}
  ...
  \If{use\_pbc}
    \For{\dbtt{aid} = \un, $\dots$, atoms}
      ...
      \For{\dbtt{axis} = \un, $\dots$, \trois}
        \State f\_coord[\dbtt{axis}] -= $\left \lfloor$ f\_coord[\dbtt{axis}] $\right\rfloor$
        \State  pixel\_pos[\dbtt{axis}] = $\left \lfloor$ f\_coord[\dbtt{axis}] $\times$ grid->n\_pix[\dbtt{axis}] $\right\rfloor$
      \EndFor
      \State pixel\_num = pixel\_pos[\un] + pixel\_pos[\deux] $\times$ grid->n\_pix[\un] + pixel\_pos[\trois] $\times$ grid->n\_xy
      \State add\_atom\_to\_pixel (\& grid->pixel\_list[pixel\_num], \cbtt{aid}, f\_coord)
    \EndFor
  \Else
    \For{\dbtt{aid} = \un, $\dots$, atoms}
      \For{\dbtt{axis} = \un, $\dots$, \trois}
	\State  pixel\_pos[\dbtt{axis}] = $\left \lfloor$ (c\_coord[\dbtt{axis}] - cmin[\dbtt{axis}]) / Cutoff $\right\rfloor$
      \EndFor
      \State pixel\_num = pixel\_pos[\un] + pixel\_pos[\deux] $\times$ grid->n\_pix[\un] + pixel\_pos[\trois] $\times$ grid->n\_xy
      \State add\_atom\_to\_pixel (\& grid->pixel\_list[pixel\_num], \cbtt{aid}, c\_coord)
    \EndFor
  \EndIf
  \Return grid
\EndProcedure
