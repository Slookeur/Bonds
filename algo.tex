...
\newcommand{\xdir}{\mbtt{x}}
\newcommand{\ydir}{\mbtt{y}}
\newcommand{\zdir}{\mbtt{z}}
...
\begin{algorithm}
\begin{algorithmic}[1]
\Procedure{Adjust PBC shift}{pixel\_grid, pixel\_coord[3], pbc\_shift[3][3][3]}
  \For{\vbtt{x\_pos} = \un, $\dots$, \trois}
    \For{\rbtt{y\_pos} = \un, $\dots$, \trois}
      \For{\obtt{z\_pos} = \un, $\dots$, \trois}
        \State pbc\_shift[\vbtt{x\_pos}][\rbtt{y\_pos}][\obtt{z\_pos}] = \zero
      \EndFor
    \EndFor
  \EndFor
  
  y
  \If{pixel\_coord[\xdir] = \xmin}
    \For{\rbtt{y\_pos} = \un, $\dots$, \trois}
      \For{\obtt{z\_pos} = \un, $\dots$, \trois}
        \State pbc\_shift[\un][\rbtt{y\_pos}][\obtt{z\_pos}] = \xmax
      \EndFor
    \EndFor
  \ElsIf{pixel\_coord[\xdir] = \xmax}
    \For{\rbtt{y\_pos} = \un, $\dots$, \trois}
      \For{\obtt{z\_pos} = \un, $\dots$, \trois}
        \State pbc\_shift[\trois][\rbtt{y\_pos}][\obtt{z\_pos}] = - \xmax
      \EndFor
    \EndFor
  \EndIf

  \If{pixel\_coord[\ydir] = \ymin}
    \For{\vbtt{x\_pos} = \un, $\dots$, \trois}
      \For{\obtt{z\_pos} = \un, $\dots$, \trois}
	\State pbc\_shift[\vbtt{x\_pos}][\un][\obtt{z\_pos}] += (\xmax $\times$ \ymax)
      \EndFor
    \EndFor
  \ElsIf{pixel\_coord[\ydir] = \ymax}
    \For{\vbtt{x\_pos} = \un, $\dots$, \trois}
      \For{\obtt{z\_pos} = \un, $\dots$, \trois}
        \State pbc\_shift[\vbtt{x\_pos}][\trois][\obtt{z\_pos}] -= (\xmax $\times$ \ymax)
      \EndFor
    \EndFor
  \EndIf

  \If{pixel\_coord[\zdir] = \zmin}
    \For{\vbtt{x\_pos} = \un, $\dots$, \trois}
      \For{\rbtt{y\_pos} = \un, $\dots$, \trois}
        \State pbc\_shift[\vbtt{x\_pos}][\rbtt{y\_pos}][\un] += (\xmax $\times$ \ymax $\times$ \zmax)
      \EndFor
    \EndFor
  \ElsIf{pixel\_coord[\zdir] = \zmax}
    \For{\vbtt{x\_pos} = \un, $\dots$, \trois}
      \For{\rbtt{z\_pos} = \un, $\dots$, \trois}
        \State pbc\_shift[\vbtt{x\_pos}][\rbtt{y\_pos}][\trois] -= (\xmax $\times$ \ymax $\times$ \zmax)
      \EndFor
    \EndFor
  \EndIf
\EndProcedure
...
\begin{algorithmic}
\Procedure{}
void add\_atom\_to\_pixel (pixel * the\_pixel, int pixel\_coord[\trois], int atom\_id, float atom\_coord[\trois])
{
  int \dbtt{axis};
...
\begin{algorithmic}
\Procedure{}
pixel\_grid * prepare\_pixel\_grid (bool use\_pbc)
{
  pixel\_grid * grid;        // pointer to the pixel grid to create
  int \dbtt{axis};                 // loop iterator axis id (0=x, 1=y, 2=z)
  int \cbtt{aid};                  // loop iterator atom number (0, atoms--1)
  int pixel\_num;            // pixel number in the grid
  int pixel\_pos[\trois];         // pixel coordinates in the grid
  float cmin[\trois], cmax[\trois];   // float coordinates min, max values
  float f\_coord[\trois];         // float fractional coordinates

  grid = malloc(sizeof*grid);
  if ( ! use\_pbc )                              // without periodic boundary conditions
  {
    for ( \dbtt{axis} = \zero ; \dbtt{axis} < \trois ; \dbtt{axis} ++ ) cmin[\dbtt{axis}] = cmax[\dbtt{axis}] = c\_coord[\zero][\dbtt{axis}];
    ...
    for ( \dbtt{axis} = \zero ; \dbtt{axis} < \trois ; \dbtt{axis} ++ )       // For x, y and z
    {
      grid->n\_pix[\dbtt{axis}] = (int)((cmax[\dbtt{axis}] - cmin[\dbtt{axis}]) / cutoff) + \un; // number of pixel(s) on axis '\dbtt{axis}'
    }
  }
  else  // using periodic boundary conditions
  {
    for ( \dbtt{axis} = \zero ; \dbtt{axis} < \trois ; \dbtt{axis} ++ )       // For x, y and z
    {
      grid->n\_pix[\dbtt{axis}] = (int)(l\_params[\dbtt{axis}] / cutoff) + \un; // number of pixel(s) on axis '\dbtt{axis}'
    }
  }
  for ( \dbtt{axis} = \zero ; \dbtt{axis} < \trois ; \dbtt{axis} ++ )         // For x, y and z
  {
    // correction if the number of pixel(s) on '\dbtt{axis}' is too small
    grid->n\_pix[\dbtt{axis}] = (grid->n\_pix[\dbtt{axis}] < 4) ? \un : grid->n\_pix[\dbtt{axis}];
  }
  ...
  grid->pixel\_list = malloc (grid->pixels*sizeof*grid->pixel\_list);
  for ( pixel\_num = \zero ; pixel\_num < grid->pixels ; pixel\_num ++ )
  {
    grid->pixel\_list[pixel\_num].pid = pixel\_num;
    grid->pixel\_list[pixel\_num].patoms = \zero;
  ...
  if ( ! use\_pbc )                              // without periodic boundary conditions
  {
    for ( \cbtt{aid} = \zero ; \cbtt{aid} < atoms ; \cbtt{aid} ++ )      // for all atoms
    {
      ...
      add\_atom\_to\_pixel (grid, pixel\_num, pixel\_pos, \cbtt{aid}, c\_coord[\cbtt{aid}]);
    }
  }
  else                                          // using periodic boundary conditions
  {
    for ( \cbtt{aid} = \zero ; \cbtt{aid} < atoms ; \cbtt{aid} ++ )      // for all atoms
    {
      ...
      for ( \dbtt{axis} = \zero ; \dbtt{axis} < \trois ; \dbtt{axis} ++ )     // for x, y and z
      {
        f\_coord[\dbtt{axis}] = f\_coord[\dbtt{axis}] - floorf(f\_coord[\dbtt{axis}]);
        pixel\_pos[\dbtt{axis}] = (int)((f\_coord[\dbtt{axis}] * grid->n\_pix[\dbtt{axis}]);
      }
      pixel\_num = pixel\_pos[\zero] + pixel\_pos[\un] * grid->n\_pix[\zero] + pixel\_pos[\deux] * grid->n\_xy;
      add\_atom\_to\_pixel (\& grid->pixel\_list[pixel\_num], \cbtt{aid}, f\_coord);
    }
  }
  return grid;
}
