...
\Procedure{prepare\_pixel\_grid}{use\_pbc, l\_params[\trois], cutoff, c\_cooord[][\trois]}
  ...
  \If{use\_pbc}
    \For{\dbtt{axis} = \un, $\dots$, \trois}
      \Sate grid->n\_pix[\dbtt{axis}] = $\left\lfloor (l\_params[\dbtt{axis}] / cutoff) \right\rfloor$ + \un
    \EndFor
  \Else
    \For{\dbtt{axis} = \un, $\dots$, \trois}
    ...
    \EndFor
    \For{\dbtt{axis} = \un, $\dots$, \trois}
      \Sate grid->n\_pix[\dbtt{axis}] = $\left\lfloor (cmax[\dbtt{axis} - cmin[\dbtt{axis}] / cutoff) \right\rfloor$ + \un
    \EndFor
  \EndIf
  \State
  \For{\dbtt{axis} = \un, $\dots$, \trois}
