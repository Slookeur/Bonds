...
MODULE parameters
  ...
  TYPE pixel
    ...
    INTEGER                                           :: neighbors       ! number of pixel neighbors for pixel
...
TYPE distance
  REAL                                                :: length          ! the distance in Å squared
...
SUBROUTINE set_pbc_shift ( grid , pixel_coord , pbc_shift )
  ...
  pbc_shift (: ,: ,:) = 0                                   ! at first there is no shift
  ...
  else if ( pixel_coord (2) .eq. grid % n_pix (2) ) then    ! pixel position on ’y’ is ma
...
SUBROUTINE prepare_pixel_grid ( use_pbc , grid )
  ...
  REAL, DIMENSION(3)                  :: cmin, cmax  ! coordinates min , max
  REAL, DIMENSION(3)                  :: f_coord     ! fractional coordinates
...
SUBROUTINE find_pixel_neighbors ( use_pbc , the_grid , the_pix )
  ...
  LOGICAL, INTENT(IN)                    :: use_pbc                 ! flag to set if PBC are used or not
  TYPE (pixel_grid), INTENT(INOUT)       :: the_grid                ! the pixel grid
  TYPE (pixel) , POINTER, INTENT(INOUT)  :: the_pix                 ! pointer to a pixel data structure
  INTEGER                                :: axis                    ! loop iterator axis id (1= x, 2= y, 3= z)
...
END SUBROUTINE evaluate_distanc
...
SUBROUTINE pixel_search_for_neighbors ( use_pbc )
  ...
  INTEGER                    :: l_start , l_end  ! nteger loop modifier
  INTEGER                    :: pid              !
  TYPE (pixel), POINTER      :: pix_i , pix_j    ! pointers on pixel data structure
  TYPE (pixel_atom), POINTER :: at_i , at_j      ! pointers on pixel_atom data structure
