...
float ** c_coord ;          // the list of Cartesian coordinates
float cutoff_squared ;      // squared value of the cutoff radius to define first neighbors
...
void set_pbc_shift ( pixel_grid * grid , int pixel_coord [3], int pbc_shift [3][3][3] )
{
  ...
  for ( x_pos = 0 ; x_pos < 3 ; x_pos ++ )
  {
    for ( y_pos = 0 ; y_pos < 3 ; y_pos ++ )
    {
      ...
      {
        pbc_shift [x_pos][y_pos][z_pos] = 0;           // at first there is no shift
      }
    }
  }
...
pixel_grid * prepare_pixel_grid ( bool use_pbc )
{
  ...
  grid -> pixel_list = malloc(grid -> pixels*sizeof*grid_pixel_list);
