...
volume = |\zero|.|\zero|                                                                  # the lattice volume
...
\subsection{Python code: adjusting the number of pixels in the grid}

\begin{lstlisting}[language=Python]
def adjust_pixel_numbers (use_pbc : bool, grid : PixelGrid, cmin : np.ndarray, cmax : np.ndarray, pixel_size : float):

  targetdp = |\mbtt{1.85}|  # target atom density per pixel
  rhonum = atoms
  if use_pbc:
  ...
  if rhonum < |\mbtt{0.01}|:
    ...
    if mpsize > |\mbtt{1.0}|:
      ...
      for |\dbtt{axis}| in range(|\trois|):                          # for x, y and z
        if use_pbc: 
          grid.n_pix[|\dbtt{axis}|] = int(l_params[axis] / pixel_size)
        else
          grid.n_pix[|\dbtt{axis}|] = int((cmax[axis] - cmin[axis]) / pixel_size)

  for |\dbtt{axis}| in range(|\trois|):                              # for x, y and z
    # correction if the number of pixels on 'axis' is too small
    grid.n_pix[|\dbtt{axis}|] = |\un| if grid.n_pix[|\dbtt{axis}|] < |\trois| else grid.n_pix[|\dbtt{axis}|]
...
def find_pixel_neighbors(use_pbc : bool, the_grid : PixelGrid, the_pix : Pixel):
  ...
  for |\vbtt{x\_pos}| in range(l_start[|\zero|], l_end[|\zero|]):
    for |\rbtt{y\_pos}| in range(l_start[|\un|], l_end[|\un|]):
      for |\obtt{z\_pos}| in range(l_start[|\deux|], l_end[|\deux|]):
        keep_neighbor = True

        if not use_pbc and boundary:
          if the_pix.p_xyz[|\zero|] == |\zero| and |\vbtt{x\_pos}| == |\zero|:
            keep_neighbor = False
          elif the_pix.p_xyz[|\zero|] == the_grid.n_pix[|\zero|]-|\un| and |\vbtt{x\_pos}| == |\deux|:
            keep_neighbor = False
          elif the_pix.p_xyz[|\un|] == |\zero| and |\rbtt{y\_pos}| == |\zero|:
            keep_neighbor = False
          elif the_pix.p_xyz[|\un|] == the_grid.n_pix[|\un|]-|\un| and |\rbtt{y\_pos}| == |\deux|:
            keep_neighbor = False
          elif the_pix.p_xyz[|\deux|] == |\zero| and |\obtt{z\_pos}| == |\zero|:
            keep_neighbor = False
          elif the_pix.p_xyz[|\deux|] == the_grid.n_pix[|\deux|]-|\un| and |\obtt{z\_pos}| == |\deux|:
            keep_neighbor = False
	    ...
	    nid += pbc_shift[|\vbtt{x\_pos}|][|\rbtt{y\_pos}|][|\obtt{z\_pos}|]
