...
MODULE parameters
  ...
  TYPE pixel
    ...
    INTEGER, DIMENSION(|\trois|)                   :: p_co            ! the pixel coordinates in the grid
    LOGICAL                                 :: tested          ! was the pixel checked already
    INTEGER                                 :: patoms          ! number of atom(s) in pixel
    TYPE(atom), DIMENSION(:), ALLOCATABLE   :: pix_atom        ! list of atom(s) in pixel, to be allocated
  ...
  TYPE grid
    ...
    TYPE (pixel), DIMENSION(:), ALLOCATABLE :: pixel_list      ! pointer to the pixels, to be allocated
  END TYPE grid
  ...
  REAL                                      :: cutoff          ! the cutoff to define atomic bond(s)
  REAL                                      :: cutoff_squared  ! squared value for the cutoff
...
\clearpage
\subsection{Commented FORTRAN90 code: set pixel periodic boundary condition shift}
\label{f-set-pshift}
\begin{lstlisting}[language={[90]FORTRAN}]
SUBROUTINE set_pbc_shift (the_grid, pixel_coord, pbc_shift)
  
  USE parameters
  IMPLICIT NONE
  
  TYPE (grid), INTENT(IN)                  :: the_grid            ! the pixel grid
  ...
  if ( pixel_coord(|\un|) .eq. |\un| ) then                               ! pixel position on 'x' is min
    pbc_shift(|\un|,:,:) = the_grid%n_pix(|\un|)
  else if ( pixel_coord(|\un|) .eq. the_grid%n_pix(|\un|) ) then          ! pixel position on 'x' is max
    pbc_shift(|\trois|,:,:) = - the_grid%n_pix(|\un|)
  endif
  
  if ( pixel_coord(|\deux|) .eq. |\un| ) then                               ! pixel position on 'y' is min
    pbc_shift(:,|\un|,:) = pbc_shift(:,|\un|,:) + the_grid%n_xy
  else if ( pixel_coord(|\deux|) .eq. the_grid%n_pix(|\deux|) ) then          ! pixel position on 'y' is max
    pbc_shift(:,|\trois|,:) = pbc_shift(:,|\trois|,:) - the_grid%n_xy
  endif
  
  if ( pixel_coord(|\trois|) .eq. |\un| ) then                               ! pixel position on 'z' is min
    pbc_shift(:,:,|\un|) = pbc_shift(:,:,|\un|) + the_grid%pixels
  else if ( pixel_coord(|\trois|) .eq. the_grid%n_pix(|\trois|) ) then          ! pixel position on 'z' is max
    pbc_shift(:,:,|\trois|) = pbc_shift(:,:,|\trois|) - the_grid%pixels
  endif
...
\clearpage
\subsection{Commented FORTRAN90 code: finding pixel neighbors}
\label{f-find-pn}
\begin{lstlisting}[language={[90]FORTRAN}]
SUBROUTINE find_pixel_neighbors (use_pbc, the_grid, the_pix)
  ...
  LOGICAL, INTENT(IN)         :: use_pbc                     ! flag to set if PBC are used or no
  TYPE (grid), INTENT(INOUT)  :: the_grid                    ! pointer to the pixel grid
  TYPE (pixel), INTENT(INOUT) :: the_pix                     ! pointer to the pixel
  ...
  if ( use_pbc ) then
    call set_pbc_shift (the_grid, the_pix%p_co, pbc_shift)
  else
    do |\dbtt{axis}| = |\un| , |\trois|
      if ( the_pix%p_co(|\dbtt{axis}|) .eq. |\un| .or. the_pix%p_co(|\dbtt{axis}|) .eq. the_grid%n_pix(|\dbtt{axis}|) ) then
        boundary = .true.
      endif
    enddo
  endif
    ...
    do |\rbtt{ypos}| = l_start(|\deux|) , l_end(|\deux|)
      do |\obtt{zpos}| = l_start(|\trois|) , l_end(|\trois|)
        ...
        if ( .not. use_pbc .and. boundary ) then
          if ( the_pix%p_co(|\un|) .eq. |\un| .and. |\vbtt{xpos}| .eq. |\un| ) then
            keep_neighbor = .false.
          else if ( the_pix%p_co(|\un|) .eq. the_grid%n_pix(|\un|) .and. |\vbtt{xpos}| .eq. |\trois| ) then
            keep_neighbor = .false.
          else if ( the_pix%p_co(|\deux|) .eq. |\un| .and. |\rbtt{ypos}| .eq. |\un| ) then
            keep_neighbor = .false.
          else if ( the_pix%p_co(|\deux|) .eq. the_grid%n_pix(|\deux|) .and. |\rbtt{ypos}| .eq. |\trois| ) then
            keep_neighbor = .false.
          else if ( the_pix%p_co(|\trois|) .eq. |\un| .and. |\obtt{zpos}| .eq. |\un| ) then
            keep_neighbor = .false.
          else if ( the_pix%p_co(|\trois|) .eq. the_grid%n_pix(|\trois|) .and. |\obtt{zpos}| .eq. |\trois| ) then
            keep_neighbor = .false.
          endif
        endif
...
\clearpage
\subsection{Commented FORTRAN90 code: preparation of the pixel grid}
...
SUBROUTINE prepare_pixel_grid (use_pbc, grid)
  ...
  TYPE (grid), INTENT(INOUT) :: grid                   ! the pixel grid to prepare
  INTEGER                    :: |\dbtt{axis}|                   ! loop iterator axis id (1=x, 2=y, 3=z)
  ...
  if ( .not. use_pbc ) then                            ! Without periodic boundary conditions
    do |\cbtt{aid}| = |\un| , atoms                                 ! For all atoms
      ...
      !   - Add atom 'aid' with coordinates 'c_coord[|\cbtt{aid}|]' to pixel 'pixel_number'
      ...
      call add_atom_to_pixel (grid, pixel_num, pixel_pos, |\cbtt{aid}|, c_coord(|\cbtt{aid}|))
    enddo
  else                                                 ! Using periodic boundary conditions
    do |\cbtt{aid}| = |\un| , atoms                                 ! For all atoms
     ...
     enddo
     pixel_num = pixel_pos(|\un|) + pixel_pos(|\un|) * grid%n_pix(|\deux|) + pixel_pos(|\trois|) * grid%n_xy + |\un|
     call add_atom_to_pixel (grid, pixel_num, pixel_pos, |\cbtt{aid}|, f_coord) ! See above
    enddo
  endif
...
\clearpage
\subsection{Commented FORTRAN90 code: inter-atomic distance calculation}
...
SUBROUTINE evaluate_distance (use_pbc, at_i, at_j, dist)
  ...
  LOGICAL, INTENT(IN)            :: use_pbc         ! flag to set if PBC are used or not
  TYPE (atom), INTENT(IN)        :: at_i            ! first pixel atom
  TYPE (atom), INTENT(IN)        :: at_j            ! second pixel atom
...
\subsection{Commented FORTRAN90 code: pixel search for first neighbor atoms}
\label{f-pc-pixel}
...
SUBROUTINE pixel_search_for_neighbors (use_pbc)
  ...
  LOGICAL, INTENT(IN)   :: use_pbc                      ! flag to set if PBC are used or not
  TYPE (grid)           :: all_pixels                   ! the pixel grid to analyze
  INTEGER               :: |\vbtt{pix}|, |\rbtt{pjx}|                     ! integer pixel ID numbers
  INTEGER               :: |\dbtt{aid}|, |\cbtt{bid}|                     ! integer loop atom numbers
  INTEGER               :: lstart, lend                 ! integer loop modifier
  INTEGER               :: pid
  TYPE (pixel), POINTER :: pix_i, pix_j                 ! pointers of pixel data structure
    ...
    if ( pix_i%patoms .gt. |\zero| ) then
      ...
      do pid = |\un| , pix_i%neighbors
        ...
        if ( pix_j%patoms .gt. |\zero| .and. .not. pix_j%tested ) then
          ...
          do |\dbtt{aid}| = |\un| , pix_i%patoms - lend
            ...
            do |\cbtt{bid}| = lstart , pix_j%patoms
