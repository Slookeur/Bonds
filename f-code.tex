...
SUBROUTINE set_pbc_shift (grid, pixel_coord, pbc_shift)
  ...
  pbc_shift(:,:,:) = |\zero|                                            ! initialization without any shift

  if ( pixel_coord(|\un|) .eq. |\zero| ) then                               ! pixel position on 'x' is min
    pbc_shift(|\un|,:,:) = grid%n_pix(|\un|)
  else if ( pixel_coord(|\un|) .eq. grid%n_pix(|\un|)-|\un| ) then             ! pixel position on 'x' is max
    pbc_shift(|\trois|,:,:) = - grid%n_pix(|\un|)
  endif
  
  if ( pixel_coord(|\deux|) .eq. |\zero| ) then                               ! pixel position on 'y' is min
    pbc_shift(:,|\un|,:) = pbc_shift(:,|\un|,:) + grid%n_xy
  else if ( pixel_coord(|\deux|) .eq. grid%n_pix(|\deux|)-|\un| ) then             ! pixel position on 'y' is max
    pbc_shift(:,|\trois|,:) = pbc_shift(:,|\trois|,:) - grid%n_xy
  endif
  
  if ( pixel_coord(|\trois|) .eq. |\zero| ) then                               ! pixel position on 'z' is min
    pbc_shift(:,:,|\un|) = pbc_shift(:,:,|\un|) + grid%pixels
  else if ( pixel_coord(|\trois|) .eq. grid%n_pix(|\trois|)-|\un| ) then             ! pixel position on 'z' is max
...
SUBROUTINE add_atom_to_pixel (the_pixel, pixel_coord, atom_id, atom_coord)
  ...
  do |\dbtt{axis}| = |\un| , |\trois|
    the_pixel%pix_atoms(the_pixel%patoms)%coord(|\dbtt{axis}|) = atom_coord(|\dbtt{axis}|)
  enddo
...
\subsection{FORTRAN90 code: adjusting the number of pixels in the grid}
\label{f-adjust-pixel-grid}
...
SUBROUTINE adjust_pixel_numbers (use_pbc, grid, cmin, cmax, pixel_size)
  ...
  REAL, DIMENSION(|\trois|), INTENT(IN)   :: cmin, cmax       ! real precision coordinates min, max
  REAL, INTENT(INOUT)              :: pixel_size       ! the initial pixel size
  REAL                             :: targetdp=|\mbtt{1.85}|    ! target atom density per pixel
  ...
  if (use_pbc) then
    ...
    do |\dbtt{axis}| = |\un| , |\trois|                    ! for x, y and z
      rhonum = rhonum / (cmax(|\dbtt{axis}|) - cmin(|\dbtt{axis}|))
    enddo
  endif

  ! if the number density if < 0.01 atom / |\blue{\AA}|^3
  if (rhonum .lt. |\mbtt{0.01}|) then
  
    rhopix = atoms
    do |\dbtt{axis}| = |\un| , |\trois|                    ! for x, y and z
      rhopix = rhopix / grid%n_pix(|\dbtt{axis}|)
    enddo
    mpsize = (targetdp/rhopix)**(|\mbtt{1.0}|/|\mbtt{3.0}|)
    if (mpsize .gt. |\mbtt{1.0}|) then
      ...
      do |\dbtt{axis}|= |\un| , |\trois|                   ! for x, y and z
        if ( use_pbc ) then
          grid%n_pix(|\dbtt{axis}|) = INT(l_params(|\dbtt{axis}|) / pixel_size)
        else
          grid%n_pix(|\dbtt{axis}|) = INT((cmax(|\dbtt{axis}|) - cmin(|\dbtt{axis}|)) / pixel_size) 
        endif
      endif
    endif
  ...
  do |\dbtt{axis}| = |\un| , |\trois|                      ! for x, y and z
    ...
    if ( grid%n_pix(|\dbtt{axis}|) .lt. |\trois| ) then
...
SUBROUTINE prepare_pixel_grid (use_pbc, grid)
  ...
  do |\cbtt{aid}| = |\un| , atoms                                   ! for all atoms
     ...
     enddo
     pixel_num = pixel_pos(|\un|) + pixel_pos(|\deux|) * grid%n_pix(|\un|) + pixel_pos(|\trois|) * grid%n_xy
     call add_atom_to_pixel (grid%pixel_list(pixel_num+|\un|), pixel_pos, |\cbtt{aid}|, f_coord)
    enddo
  else                                                 ! without periodic boundary conditions
    do |\cbtt{aid}| = |\un| , atoms                                 ! for all atoms
      do |\dbtt{axis}| = |\un| , |\trois|                                  ! for x, y and z
        if (grid%n_pix(|\dbtt{axis}|) .eq. |\un|) then
          pixel_pos(|\dbtt{axis}|) = |\zero|
        else
          pixel_pos(|\dbtt{axis}|) = INT((c_coord(|\cbtt{aid}|,|\dbtt{axis}|) - cmin(|\dbtt{axis}|))/pixel_size)
        endif
      enddo
      pixel_num = pixel_pos(|\un|) + pixel_pos(|\deux|) * grid%n_pix(|\un|) + pixel_pos(|\trois|) * grid%n_xy
      call add_atom_to_pixel (grid%pixel_list(pixel_num+|\un|), pixel_pos, |\cbtt{aid}|, c_coord(|\cbtt{aid}|,:))
    enddo
  endif
...
SUBROUTINE find_pixel_neighbors (use_pbc, the_grid, the_pix)
  ...
  INTEGER                              :: |\dbtt{axis}|                    ! loop iterator axis id (1=x, 2=y, 3=z)
  INTEGER                              :: |\vbtt{x\_pos}|, |\rbtt{y\_pos}|, |\obtt{z\_pos}|     ! neighbor position on x, y and z
  ...
  if ( use_pbc ) then
    ...
    do |\dbtt{axis}| = |\un| , |\trois|
      if ( the_pix%p_xyz(|\dbtt{axis}|) .eq. |\zero| .or. the_pix%p_xyz(|\dbtt{axis}|) .eq. the_grid%n_pix(|\dbtt{axis}|) - |\un| ) then
        boundary = .true.
      endif
    enddo
  endif
  ...
  enddo
  |\bftt{nnp}| = |\zero|
  do |\vbtt{x\_pos}| = l_start(|\un|) , l_end(|\un|)
    do |\rbtt{y\_pos}| = l_start(|\deux|) , l_end(|\deux|)
      do |\obtt{z\_pos}| = l_start(|\trois|) , l_end(|\trois|)
        keep_neighbor = .true.
        if ( .not. use_pbc .and. boundary ) then
          if ( the_pix%p_xyz(|\un|) .eq. |\zero| .and. |\vbtt{x\_pos}| .eq. |\un| ) then
            keep_neighbor = .false.
          else if ( the_pix%p_xyz(|\un|) .eq. the_grid%n_pix(|\un|)-|\un| .and. |\vbtt{x\_pos}| .eq. |\trois| ) then
            keep_neighbor = .false.
          else if ( the_pix%p_xyz(|\deux|) .eq. |\zero| .and. |\rbtt{y\_pos}| .eq. |\un| ) then
            keep_neighbor = .false.
          else if ( the_pix%p_xyz(|\deux|) .eq. the_grid%n_pix(|\deux|)-|\un| .and. |\rbtt{y\_pos}| .eq. |\trois| ) then
            keep_neighbor = .false.
          else if ( the_pix%p_xyz(|\trois|) .eq. |\zero| .and. |\obtt{z\_pos}| .eq. |\un| ) then
            keep_neighbor = .false.
          else if ( the_pix%p_xyz(|\trois|) .eq. the_grid%n_pix(|\trois|)-|\un| .and. |\obtt{z\_pos}| .eq. |\trois| ) then
            keep_neighbor = .false.
          endif
        endif
        if ( keep_neighbor ) then
          ! evaluating neighbor pixel number in the grid
          nid = the_pix%pid + pmod(|\vbtt{x\_pos}|) + pmod(|\rbtt{y\_pos}|) * the_grid%n_pix(|\un|) + pmod(|\obtt{z\_pos}|) * the_grid%n_xy
          if ( use_pbc ) then
            ! corrections if required in the case where PBC are applied
            nid = nid + pbc_shift(|\vbtt{x\_pos}|, |\rbtt{y\_pos}|, |\obtt{z\_pos}|)
          endif
