...
SUBROUTINE add_atom_to_pixel (the_pixel, pixel_coord, atom_id, atom_coord)
  ...
  if (allocated(the_pixel%pix_atoms)) then
    the_pixel%pix_atoms = realloc(the_pixel%pix_atoms, the_pixel%patoms)
  else
...
SUBROUTINE evaluate_distance (use_pbc, at_i, at_j, dist)
  ...
  LOGICAL, INTENT(IN)              :: use_pbc         ! flag to set if PBC are used or not
  TYPE (atom), POINTER, INTENT(IN) :: at_i            ! first pixel atom
  TYPE (atom), POINTER, INTENT(IN) :: at_j            ! second pixel atom
...
SUBROUTINE pixel_search_for_neighbors (use_pbc)
  ...
  INTEGER               :: l_start, l_en                ! integer loop modifier
  INTEGER               :: pid
